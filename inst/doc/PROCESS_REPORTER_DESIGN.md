...
collect_process_metrics <- function(
  ...
  # Note on PID Reuse:
  #   Does NOT validate process start time. Extremely rare PID reuse could result
  #   in monitoring wrong process. Risk accepted for implementation simplicity.
  #   If original process dies, PROCESS_DIED error caught within 10 seconds.
  ...
  #     - error_type = PROCESS_DIED | PERMISSION_DENIED | PS_ERROR | 
...
task_start <- function(..., auto_start_reporter = TRUE) {
  ...
  if (auto_start_reporter) {
    ...
    if (is.null(reporter_status)) {
      ...
      # Wait briefly for reporter to start
      Sys.sleep(2)
      
      # Verify it started
      if (is.null(get_process_reporter_status())) {
        warning("Failed to start process reporter - metrics will not be collected")
      }
    } else {
...
- [ ] Implement `collect_process_metrics()` using ps package with:
  ...
  - [ ] Return run_id in metrics data
- [ ] Implement error detection and classification (all error types)
...
- [ ] Auto-start integration in `task_start()`
...
- **ps** package (>= 1.7.0) - Cross-platform process information
...
**Note on PID Reuse:** The design does not validate process start times to detect PID reuse. This extremely rare condition (process dies, system cycles through PID space, new process reuses PID within 10-second monitoring interval) could theoretically result in monitoring the wrong process. The risk is accepted for implementation simplicity, as the original process death would be caught immediately by PROCESS_DIED error detection.
